...
All 4 *should* be used during each pentest.
...
Maintain access.
Often performed before Information Gathering and Pillaging.
...
Bring the data home.
Used to test if countermeasures are working (e.g., EDR detection).
...
Is the client ransomware-proof?
...
Once paths are found, prioritize them. We can jump to the step where we use these vulnerabilities to access other systems.
